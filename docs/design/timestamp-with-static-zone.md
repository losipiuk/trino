...
| `date` | `static(0, Z)` | midnight in Z (mirrors `date → timestamp with time zone`, which uses the session zone; see §8 Q3) |
| `timestamp(p)` | `static(p, Z)` | **not implicit** in v1. `timestamp → timestamp with time zone` remains the implicit path; it uses the session zone. |
...
**Recommendation: Z2.** The zone is a property of the type, the descriptor is what
views persist (`TypeId` is the descriptor's SQL text, `TypeRegistry.getType(TypeId)`
parses it with the SQL grammar), so it must be readable and parseable. Z1 stays as a
fallback if the SPI change is contested. Z3 is rejected because Piotr's stated end goal
includes `current_timestamp` (session zone) and `AT TIME ZONE` producing the type.
...
- `compatibility()` same-base branch keys off the base name only, so it must inspect Z:
  same Z → widen precision; different Z → common supertype
  `timestamp(max p) with time zone`, not coercible either way.
- `coerceTypeBase`: `static → timestamp with time zone` (keep p); `date → static`?
  see §8 Q3.
...
- `DESCRIBE OUTPUT`, JDBC `getColumnTypeName`, `information_schema.columns`,
  `SHOW COLUMNS`, `DESCRIBE`, `SHOW CREATE TABLE`, `typeof()`: these are varchar and
  will show the new name. This is the one place the type is user-visible in v1. See §8
  Q4 for whether to mask it in `information_schema`/`SHOW CREATE TABLE`.
...
- **Q2. Zone parameter encoding.** Z2 (new `TypeParameter.Text`) vs Z1 (numeric key).
  Recommendation Z2.
- **Q3. `date → static` implicit coercion.** `date → timestamp with time zone` is
  implicit today and uses the session zone. `date → static(Z)` would be midnight in Z,
  which is a different instant when Z ≠ session zone. Option: no implicit coercion,
  comparisons `static_col = DATE '...'` resolve through the tz path (session zone),
  same as today. Recommendation: no implicit `date → static`, keep behaviour identical
  to today, and let `UnwrapCastInComparison` handle `CAST(static AS date) = DATE`.
- **Q4. Visibility in metadata.** Should `information_schema.columns`,
  `SHOW CREATE TABLE`, JDBC `DatabaseMetaData` keep reporting
  `timestamp(6) with time zone` for static columns (a "technical type", per Piotr), or
  report the real type? Reporting the real type is honest and round-trips through
  `CREATE TABLE`; masking it hides the change from BI tools that switch on type names.
  Recommendation: report the real type; the release note calls it out.
...
- **Q6. Precision split.** p ≤ 6 as `long` (like `timestamp`) is proposed. Confirm
  there is no reason to keep parity with tz's p ≤ 3 split. (None found.)
- **Q7. `timestamp → static` implicit coercion.** Kept non-implicit so the session-zone
  interpretation stays explicit via the tz path. Confirm.
- **Q8. Which functions get native overloads in v1** vs rely on the coercion to tz.
  Proposal: all in §4.4, since the pack-per-row coercion would negate the execution win
  for the exact functions the planner wants to unwrap.
