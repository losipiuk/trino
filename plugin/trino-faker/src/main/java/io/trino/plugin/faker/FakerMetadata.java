        ...
        implements ConnectorMetadata
{
    ...
    public synchronized TableStatistics getTableStatistics(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        ...
        tableStatisitics.setRowCount(Estimate.of((long) info.properties().get(TableInfo.DEFAULT_LIMIT_PROPERTY)));
        ...
        info.columns().forEach(columnInfo -> {
            ...
            Collection<?> allowedValues = (Collection<?>) PropertyValues.propertyValue(columnInfo.metadata(), ALLOWED_VALUES_PROPERTY);

            if (allowedValues != null && (min != null || max != null)) {
                throw new IllegalStateException("The `%s` property cannot be set together with `%s` and `%s` properties".formatted(ALLOWED_VALUES_PROPERTY, MIN_PROPERTY, MAX_PROPERTY));
            }
