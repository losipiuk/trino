...
import io.trino.execution.scheduler.StageEventDrivenTaskSourceFactory.ArbitraryDistributionSplitAssigner;
import io.trino.execution.scheduler.StageEventDrivenTaskSourceFactory.SplitAssigner;
...
public class TestArbitraryDistributionSplitAssigner
{
    ...
    public void testEmpty()
    {
        ...
        SplitAssigner splitAssigner = createSplitAssigner(ImmutableSet.of(PARTITIONED_1), ImmutableSet.of(), 100);
        ...
        splitAssigner = createSplitAssigner(ImmutableSet.of(), ImmutableSet.of(REPLICATED_1), 100);
        ...
        splitAssigner = createSplitAssigner(ImmutableSet.of(PARTITIONED_1), ImmutableSet.of(REPLICATED_1), 100);
        callback = new TestingTaskSourceCallback();
        splitAssigner.assign(REPLICATED_1, ImmutableListMultimap.of(), true).update(callback);
        splitAssigner.assign(PARTITIONED_1, ImmutableListMultimap.of(), true).update(callback);
        splitAssigner.finish().update(callback);
        ...
        splitAssigner = createSplitAssigner(ImmutableSet.of(PARTITIONED_1, PARTITIONED_2), ImmutableSet.of(REPLICATED_1, REPLICATED_2), 100);
        callback = new TestingTaskSourceCallback();
        splitAssigner.assign(REPLICATED_1, ImmutableListMultimap.of(), true).update(callback);
        splitAssigner.assign(PARTITIONED_1, ImmutableListMultimap.of(), true).update(callback);
    ...
    private void fuzzTesting(boolean withHostRequirements)
    {
        ...
        testAssigner(partitionedSources, replicatedSources, batches, splitsPerPartition);
    }
    ...
    {
        SplitAssigner splitAssigner = createSplitAssigner(partitionedSources, replicatedSources, STANDARD_SPLIT_SIZE_IN_BYTES * partitionedSplitsPerPartition);
        TestingTaskSourceCallback callback = new TestingTaskSourceCallback();
